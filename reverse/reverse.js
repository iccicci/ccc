	...
	bc.stdout.on("data", data => {
		if(data.toString().indexOf("Listening") !== -1) {
			...
			web3.eth.getAccounts((err, res) => {
				...
				const tx = new web3.eth.Contract(JSON.parse(abi)).deploy({ arguments: [], data: bin }).send({ from: res[0], gas: 4700000, gasPrice: "30000000000" });
				tx.on("receipt", receipt => {
					console.log("Deploy   tx:     ", receipt.transactionHash);
					console.log("Contract address:", receipt.contractAddress);

					const contract = new web3.eth.Contract(JSON.parse(abi), receipt.contractAddress);
					...
					contract.methods
						.f(web3.utils.hexToBytes(web3.utils.utf8ToHex(". ccc! .")), 1)
						.send({ from: res[0], gas: 4700000, gasPrice: "30000000000" })
						.on("transactionHash", tx => {
							console.log("f()      tx:     ", tx);

							contract.methods.f(web3.utils.hexToBytes(web3.utils.utf8ToHex(". ccc! .")), 0).call({}, (err, res) => {
