...
class Parser {
	...
	parse(context, res) {
		try {
			...
			this[context.attr || context.typedef ? "declaration" : "preModifier"](context);
	...
	declaration(context) {
		...
		const parse = this[context.param || context.typeonly ? "reference" : "comma"].bind(this, context);
		const pos = this.pos;
	...
	assignment(context) {
		...
		while(["=", "+=", "-=", "*=", "/=", "%=", "<<=", ">>=", "&=", "^=", "|="].indexOf((current = this.current()).token) !== -1) {
			...
			const right = this.arithmetic(subContext, nrule);
	...
	arithmetic(context, n) {
		const constant = true;
		const rule = rules[n];
		let expression, current, hex, lhex, number, op, rhex, right, t, token, type, v;
		let left = rule.left ? this.arithmetic(context, n - 1) : {};

		const checkNumber = (expression, left, unary) => {
			if(! expression.type.integer) this.error(current, `invalid ${left ? "left" : "right"} operand of type '${expression.type.type}' to ${unary ? "unary" : "binary"} operator '${t}'`);
		...
		const operate = op => (left = left.constant && right.constant ? { constant, first: left.first, hex: toHex(ops[op]()), number, type } : { first: left.first, left, op, right, number, type });

		while(this.is(rule, (current = this.current()))) {
			this.advance();
			right = rule.right ? this.arithmetic(context, n - 1) : {};
			...
			case "addiction":
				...
				if(left.type.pointer) {
					if(right.type.pointer) this.error(current, `invalid operands of type '${left.type.type}' and '${right.type.type}' to binary operator '${t}'`);
					checkNumber(right);
					if(right.constant) right.hex = toHex(rhex.mul(x20));
					else right = { ...right, left: { hex: x20 }, op: "mul", right };
					type = left.type;
				}
				else {
					checkNumber(left, true);
					if(right.type.pointer) {
						if(t === "-") checkNumber(right);
						if(left.constant) left.hex = toHex(lhex.mul(x20));
						else left = { ...left, left, op: "mul", right: { hex: x20 } };
						type = right.type;
					}
					else {
						checkNumber(right);
			...
			case "sign":
				...
				({ hex, type } = right);
				op = "nop";
				if(t === "+" && right.type.pointer) number = false;
				else {
					number = checkNumber(right, false, true);
					if(t === "-") {
						op = "neg";
				...
				return right.constant ? { constant, hex, number, type } : { number, op, right, type };
			}
		}

		if(rule.type === "sign") return this.square(context);
	...
	literal(context) {
		...
		if((somthing = scope.getVariable(token))) return { ...somthing, first };
