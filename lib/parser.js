...
class Parser {
  ...
  assignment(context) {
    ...
    if(["=", "+=", "-=", "*=", "/=", "%=", "<<=", ">>=", "&=", "^=", "|="].indexOf((current = this.current()).token) !== -1) {
      ...
      if(! lvalue) this.error(context, current, `left operand of operator '${current.token}' is not an lvalue`);
  ...
  literal(context) {
    ...
    if((somthing = scope.getVariable(token))) return this.postOperand(context, { ...somthing, first });
    this.error(context, first, `undefined symbol: '${token}'`);
  }

  postOperand(context, left) {
    const current = this.current();
    const { first, lvalue, type } = left;
    const { token } = current;
    let right, step;
    ...
    case "--":
      ...
      return this.postOperand(context, { first, left, op: token, step, type });

    case "[":
      this.advance();
      if(! left.type.pointer) this.error(current, `cannot indext type '${left.type.type}'`);
      right = this.assignment(context);
      if(left.type.index ? left.type.index !== right.type.type : ! right.type.integer) this.error(current, `type '${left.type.type}' cannot be indexed with type '${right.type.type}'`);
      this.consume("]");

      return this.postOperand(context, { first, left, op: "index", right, type: type.deref });
    }
